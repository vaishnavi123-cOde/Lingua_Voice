...
class APIService:
    ...
    def get_enhanced_word_info(self, word: str, language: str = 'en') -> Dict:
        ...
        Priority: OpenAI API > Free Dictionary API > Offline Dictionary > Fallback
        """
        result = {
            'word': word,
            'language': language,
        ...
        # Try OpenAI API first (best for all languages)
        try:
            import openai_service
            if openai_service.openai_service:
                openai_result = openai_service.openai_service.get_word_meaning(word, language)
                if openai_result and openai_result.get('definition'):
                    result['definition'] = openai_result['definition']
                    result['phonetic'] = openai_result.get('pronunciation', '')
                    result['examples'] = [openai_result.get('example', '')] if openai_result.get('example') else []
                    result['synonyms'] = openai_result.get('synonyms', [])
                    result['source'] = 'openai'

                    print(f"[API_SERVICE] Got meaning from OpenAI for {word} ({language}): {result['definition'][:50]}...")
                    return result
        except Exception as e:
            print(f"[API_SERVICE] OpenAI error for {word}: {e}")
